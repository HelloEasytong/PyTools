...
class Ui_about_window(object):
    ...
    def retranslateUi(self, about_window):
        ...
        self.uiGroupBox_app_name.setTitle(_translate("about_window", "PyTools"))
        self.uiLabel_app_description.setText(_translate("about_window", "PyTools 是一个关于 Python 的工具箱程序，主要是为 Python 的包管理工具 pip 和 Python 程序打包工具 PyInstaller 封装图形用户界面，目前提供了 5 个工具：Python 包管理器、程序打包工具、Pip 源设置工具、模块安装包下载器、云函数部署包打包工具。"))
        self.label.setText(_translate("about_window", "程序版本："))
        self.uiLabel_app_version.setText(_translate("about_window", "PyTools - 1.0.0"))
